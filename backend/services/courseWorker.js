  ...
  async (job) => {
    ...
    try {
      if (type === 'generateOutline') {
        ...
        const { title: courseTitle, description: courseDescription, estimatedDuration, chapters: outline } = await generateCourseOutline(topic, depth);
        if (!outline || outline.length === 0) {
          throw new Error('LLM returned an empty outline. Aborting to prevent hang.');
        }

        // Persist course-level metadata generated by the LLM
        await prisma.course.update({
          where: { id: courseId },
          data: {
            title: courseTitle || topic,        // fallback to topic if LLM omits it
